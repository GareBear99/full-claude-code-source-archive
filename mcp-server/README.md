# Gary Doman Claude Code Source Explorer MCP

Maintained by **Gary Doman / GareBear99**.

Public-facing MCP explorer for Gary Doman's archive repo
...
```json
{
  "mcpServers": {
    "claude-code-explorer": {
      "command": "node",
      "args": ["/absolute/path/to/claude-code-source-archive/mcp-server/dist/index.js"],
      "env": {
        "CLAUDE_CODE_SRC_ROOT": "/absolute/path/to/claude-code-source-archive/src"
      }
    }
  }
}
...
```bash
# From repo root
docker build -f mcp-server/Dockerfile -t claude-code-source-archive-mcp .
docker run -p 3000:3000 -e MCP_API_KEY=your-secret claude-code-source-archive-mcp
```
...
Registry name: `io.github.garebear99/claude-code-source-explorer-mcp`
